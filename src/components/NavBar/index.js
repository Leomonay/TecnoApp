...
function PasswordForm({close}){
  ...
  useEffect(()=>confirm && newPassword && setError(confirm!==newPassword), [confirm, newPassword])

  function handleSubmit(e){
    e.preventDefault()
    dispatch(peopleActions.update(userData.id, {currentPassword:password, newPassword: newPassword}))
  ...
  return <div className="modal text-dark">
    <form className="col-sm-6 px-3 rounded bg-light shadow-sm" onSubmit={handleSubmit}>
      ...
      <div className="row my-2">
        {error ? 
          ...
          :<div className="col d-flex justify-content-center">
            <input className="btn btn-primary" type='submit' title='Cambiar' disabled={!newPassword || !confirm || newPassword!==confirm}/>
          </div>}
      </div>
      {peopleResult.error && <ErrorModal message={peopleResult.error} close={()=>dispatch(peopleActions.resetResult())}/>}
      {peopleResult.success && <SuccessModal message={'Contraseña actualizada exitosamente'} close={handleCloseSuccess} />}
    </form>
  </div>
}
      ...
      <div className="container-fluid p-0">
        <div className="row m-0">
          <div className="col-sm-auto d-flex p-0 justify-content-between">
            <Link className="mx-1 my-0 p-0" to='/panel' onClick={()=>setVisible(false)}>
              <img className='navBarLogo navbar-brand' src={logo} alt=''/>
            </Link>
            {pass&&<PasswordForm close={()=>setPass(!pass)}/>}
            <button className="btn btn-outline-dark toggleMenu my-auto" 
              onClick={handleToggle}
              style={{height: 'fit-content'}}>
              <i className="fas fa-bars"/>
            </button>
          </div>
          <div className={`col-sm-auto p-0 d-flex flex-grow-1 navBarLinkContainer ${visible ? 'visibleNavBar' : ''}`}>
              ...
              <div className="row m-0">
                ...
                <div className="col-sm-auto p-0 d-grid gap-2">
                  {isAdmin&& <NavLink to={'/admin'}
                    onClick={()=>setVisible(false)}
                    className={(navData) => `btn nav-item navAdminLink ${navData.isActive ? "activeAdminLink" :''}`}>
                    Menú Admin
                  </NavLink>}
                </div>

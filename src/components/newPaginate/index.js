...
export default function NewPaginate({length, visible, size, select}){
    ...
    const [lastPage, setLastPage]=useState(Math.ceil(Number(length)/pageSize))

    useEffect(()=>console.log('pageSize', pageSize),[pageSize])

    useEffect(()=>{
        const delta = Math.floor(Number(visible)/2)
        let min = Math.max(1, current-delta)-Math.max(delta-(lastPage-current),0)
        let max = Math.min (min +2*delta )
        let arrayIndex = []
            for (let i= min; i<=max; i++) if(i>=1 && i<=lastPage) arrayIndex.push(i)
    ...
    function handleSize(e){
        ...
        console.log('size', size, 'last', last)
        setLastPage(last)
        setPageSize(size)

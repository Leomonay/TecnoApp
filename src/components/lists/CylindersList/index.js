      ...
      <table className="table table-striped" style={{textAlign:'center', marginLeft: '1rem', marginRight: '1rem'}}>
        ...
        <tbody>
          {cylinders[0] && cylinders.map( (element, index)=>{
            const user = element.user? workers.find(e=>e.id === element.user.id) : 'STOCK'
          return <tr key={index}>
            ...
            <td className={user ? '' : 'crossedUp'}>
              ...
              {user?'':<i className="fas fa-ban" style={{color: 'darkred'}} title='inactivo'/>}
            </td>
            <td>{element.status}</td>
            <td>{element.givenDate?(new Date (element.givenDate)).toLocaleDateString():'-'}</td>

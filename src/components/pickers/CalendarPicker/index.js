...
export default function CalendarPicker(props){
    ...
    useEffect(()=>setStringDates(taskDates.map(date=>{
        ...
        return date.toLocaleDateString().split(' ')[0]})),[taskDates])
    ...
    async function setAllDates(event){
        ...
        datesInput=[...datesInput,...dates.filter(date=>!!(date.orders&&date.orders[0]))]
        setTaskDates(datesInput.map(item=>new Date (item.date)))
        setStringDates(datesInput.map(item=>(new Date (item.date)).toLocaleDateString().split(' ')[0]))
    ...
    return<tr style={{fontSize: '80%'}}>
            <td className='p-0'>
                <div>
                    <div  style={{minWidth: 'fit-content'}}><b>{task.name}</b></div>
                    <div>{task?
                        (`${task.strategy} ${task.responsible?
            ...
            {months.map((item, index)=>
                <td key={index} id={index}  className='p-0' style={{
                    borderTop: 'none',
                    borderBottom: 'none',
                    ...
                    <div className='d-flex align-content-center w-100 justify-content-evenly'
                        ...
                        {yearDates.filter(date=>(new Date(date)).getMonth() === index).map((element)=>{
                            ...
                            const dateObject = dates.find(item=>item.date===element.toISOString())
                            const orders = dateObject && dateObject.orders
                            if(orders && orders[0])console.log(dateObject)
                            
                            return <button key={weekIndex}
                                className={`btn p-0 my-auto ${stringDates.includes(date)? 
                                    (orders && orders[0]?'btn-warning':'btn-success')
                                    :'btn-outline-success'}`}
                                style={{height: '1rem', width: '.8rem'}}
                                value={weekIndex}
                                onClick={(e)=>setAllDates(e)}
                                title={date.split('/').splice(0,2).join('/') + (orders && orders[0]?` - tiene órdenes asociadas: ${orders.join(', ')}`:'')}
                            />
                        })}
                    </div>
                </td>

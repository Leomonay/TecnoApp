...
export default function Device(){
    ...
    useEffect(()=>{
        ...
        if(!orders) return
        setReclaims(orders.filter(order => order.class === "Reclamo")
            .map(order=>( {...order,date: new Date (order.date)} )) )
        setInterventions(history.map(task=>({...task,date:new Date (task.date)})).sort((a,b)=>a.date>b.date?-1:1) )
    },[deviceHistory])
    useEffect(()=>{const monthReclaims = []
            for (let i=0; i<=11; i++) monthReclaims[i]=reclaims.filter(reclaim=>
                reclaim.date.getFullYear() === year && reclaim.date.getMonth() === i).length
            setValues([{
                label: year,
                data: monthReclaims,
    ...
    return(
        <div className='pageBackground'>
            {!code ? <DeviceList plant={userData.plant}/>:
            <div className="container">
                <div className="row text-center">
                    <h3>{`[${device.code}] ${device.name}`}</h3>
                </div>
                <div className="row">
                    <div className="col">
                        <div className="container">
                            ...
                            <FormInput key={device.plant} label={<div><i className="fas fa-map-marker-alt"/></div>} defaultValue={`${device.plant} > ${device.area} > ${device.line}`} readOnly={true} />
                            <FormInput label='Tipo' defaultValue={device.type} readOnly={true} />
                            <FormInput key={device.power} label='Potencia' 
                                defaultValue={device.power >= 9000 ?
                                    `${Math.floor(device.power/3000)} TnRef`
                                    :`${device.power} Frigorías`} readOnly={true} />
                            <FormInput label='Gas' defaultValue={device.refrigerant} readOnly={true} />
                            ...
                            <FormInput label='Ambiente' defaultValue={device.environment} readOnly={true} />
                            <FormInput key={device.age}
                                label='Antigüedad' defaultValue={`${device.age? `${device.age} años `:''} ${ device.regDate? `(${device.regDate.split('T')[0]})` :''}`} readOnly={true} />
                    ...
                    <div className="col col-md-6">
                        <div className='container'>
                            ...
                            <div className="flex">
                                ...
                                <button key={chartValues+index} className={`yearBox ${chartValues.find(value=>value.label===year) ? 'activeYearBox' :''}`} style={{'--main-color':colors[index]}}
                                    onClick={handleClickYear} id={index} value={year}>
                                    {year}
                                </button>)}
                            </div>
                            <div className="row">
                ...
                <div className="row">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Uso de gas</th>
                            <th scope="col">Orden de Trabajo</th>
                        </tr>
                    </thead>
                    <tbody>
                        {interventions[0] && interventions.map((intervention, index)=>{
                            return(
                                <tr key={index}>
                                    <th style={{minWidth: '5rem'}}>{intervention.date.toLocaleDateString()}</th>
                                    <td>{intervention.workers.map(worker=>worker.name).join(', ')}</td>
